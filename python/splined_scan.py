...
def inspect_local_preflight(
    ...
    # WEBP has explicit precedence.  It is preserved as source material and a
    # canonical still JPEG is produced from it without remote source discovery.
    for path in files["webp"]:
        try:
            candidate = inspect_candidate_file(path, "local", -2)
            return {
                "action": "webp",
                "candidate": candidate,
                "fallback": [],
    ...
    if static_candidates:
        ...
        cleanup = [candidate.path for candidate in static_candidates if candidate.path != best.path]
        projected = project_candidate(best, cfg, format_order)
        return {
            "action": "local-ideal" if projected["range_type"] == "Ideal" else "fallback",
            "candidate": best,
            "fallback": [] if projected["range_type"] == "Ideal" else [best],
            "cleanup": cleanup,
            "diagnostics": diagnostics,
        }
    ...
    if album.audio_files:
        ...
        if candidate is not None:
            projected = project_candidate(candidate, cfg, format_order)
            return {
                "action": "embedded-ideal" if projected["range_type"] == "Ideal" else "fallback",
                "candidate": candidate,
                "fallback": [] if projected["range_type"] == "Ideal" else [candidate],
                "cleanup": [],
                "diagnostics": diagnostics,
            }
...
def apply_local_preflight(
    ...
    print(
        ...
        f"{core.white('squared=')}{core.bool_color(bool(info['square']))}"
        f"{core.white(' · resized=')}{core.bool_color(bool(info['resized']))}"
        f"{core.white(' · converted=')}{core.bool_color(bool(info['converted']))}"
...
def run_scan_dir(
    ...
    cache = core.resolve_path(config_file, str(scan.get("cache_dir", "cache")))
    ...
    print(
        ...
        + core.bracketed_text(core.format_timeout_hours(timeout_hours) + "h", core.green)
